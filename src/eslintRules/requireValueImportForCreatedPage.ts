 ...
 * the specifier is gone, resolution falls back to the kebab-cased sibling
 * module, and a page in any other directory throws `Unknown page`. Neither
 * `tsc` nor the playground (which executes the `.ts`, import still present)
 * can catch it — only a cloud run does, which is why this is an `error`.
 *
 ...
 * specifiers are never followed — a type-only one of those falls back to the
 * sibling convention on every platform alike, so it is not reported.
 */
export const requireValueImportForCreatedPageRule: PomLintRule = {
  module: {
    ...
    meta: {
      messages: {
        typeOnlyImport:
          '`{{name}}` is bound by a type-only import, which compilation erases. `create("{{name}}")` resolves the name by reading this file\'s imports at runtime, so on the cloud runner the compiled file no longer says where `{{name}}` lives and the call throws `Unknown page` unless the module happens to sit next to this file. Drop the `type` keyword — a value import survives compilation — or construct the page directly with `new {{name}}(page)`.',
      },
    },
  },
 ...
 * `undefined` means the name is not imported at all — the legitimate
 * sibling-convention path, which one file cannot judge.
 */
